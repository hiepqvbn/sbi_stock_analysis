...
def update_analysis(stock_code, expected_price):
    ...
    # Show all names used for this code
    names = stock_df["stock_name"].unique()
    ...
    perf = analyze_stock_performance(
        stock_df, names_str, current_price, plot=True)

    metrics = html.Div([
        html.Div(f"Stock code: {stock_code}"),
        html.Div(f"Names used: {names_str}"),
        html.Ul([
            html.Li(f"Total Buy Qty: {perf['total_buy_qty']}"),
            html.Li(f"Total Buy Amount: {perf['total_buy_amount']:,}"),
            html.Li(f"Avg Buy Price: {perf['avg_buy_price']:.2f}"),
            html.Li(f"Total Sell Qty: {perf['total_sell_qty']}"),
            html.Li(f"Total Sell Amount: {perf['total_sell_amount']:,}"),
            html.Li(
                f"Realized Profit: {perf['realized_profit']:.2f} ({perf['realized_profit_pct']:.2f}%)"),
            html.Li(
                f"Unrealized Profit: {perf['unrealized_profit']:.2f} ({perf['unrealized_profit_pct']:.2f}%)"),
            html.Li(
                f"Total Profit: {perf['total_profit']:.2f} ({perf['total_profit_pct']:.2f}%)"),
        ])
    ])

    price_fig = perf.get("plot", {})
    indicators_fig = {}
    ...
    return metrics, price_fig, indicators_fig, table
